...
fn test_set_and_get_affinity() {
    let cpu = match cpu_affinity(None) {
        Ok(cpus) if !cpus.is_empty() => cpus[0],
        _ => return,
    };
    ...
    match result {
        ...
        Err(CpuAffinityError::Io(ref err))
            if err.raw_os_error() == Some(1) || err.raw_os_error() == Some(22) =>
        {
            eprintln!("Skipping affinity test: insufficient permissions or restricted cpuset");
        }
...
fn test_affinity_with_multiple_cpus() {
    let available = match cpu_affinity(None) {
        Ok(cpus) if cpus.len() >= 2 => cpus,
        _ => return,
    };
    ...
    if let Err(e) = result {
        match e {
            CpuAffinityError::Io(ref err)
                if err.raw_os_error() == Some(1) || err.raw_os_error() == Some(22) =>
            {
                eprintln!("Skipping: insufficient permissions or restricted cpuset");
            }
...
fn test_physical_cores_affinity() {
    if let Ok(mapping) = core_to_cpus_mapping() {
        if let Some(&first_core) = mapping.keys().next() {
            let expected_cpu = mapping[&first_core].first().copied();
            ...
            if let Err(e) = result {
                match e {
                    CpuAffinityError::Io(ref err)
                        if err.raw_os_error() == Some(1) || err.raw_os_error() == Some(22) =>
                    {
                        eprintln!("Skipping: insufficient permissions or cpuset restriction");
                    }
                    e => panic!("Unexpected error: {e:?}"),
                }
            }
...
fn test_affinity_deduplication() {
    ...
    let result = std::thread::spawn(move || {
        set_cpu_affinity(None, [cpu, cpu, cpu])?;
        let affinity = cpu_affinity(None)?;
        assert_eq!(affinity, vec![cpu], "Duplicates should be deduplicated");
        Ok::<(), CpuAffinityError>(())
    })
    .join()
    .expect("Thread panicked");
